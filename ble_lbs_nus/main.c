...
#define DEVICE_NAME                     "Primate Optogenetics"                  /**< Name of device. Will be included in the advertising data. */
...
#define CONTROL_LED                     17                         /**< LED to be toggled with the help of the LED Button Service. */
...
//        NRF_LOG_INFO("ADC event number: %d", (int)m_adc_evt_counter);
        ...
        for (int i = 0; i < SAMPLES_IN_BUFFER; i++)  {
            ...
            adc_value = p_event->data.done.p_buffer[i] * 3.6 / 4096;
        }
 //       m_adc_evt_counter++;

 //       adc_value = adc_value/SAMPLES_IN_BUFFER;
...
//          bytes_to_send = sprintf(nus_string, "CH0: %d\r\n", p_event->data.done.p_buffer[0]);
        bytes_to_send = sprintf(nus_string, "Voltage Read: " NRF_LOG_FLOAT_MARKER "\r\n", NRF_LOG_FLOAT((p_event->data.done.p_buffer[0]) * 3.6 / 4096));                     
                                                          
        err_code = ble_nus_data_send(&m_nus, nus_string, &bytes_to_send, conn_handle);
...
{
    nrf_gpio_cfg_output(CONTROL_LED);
...
static void led_write_handler(uint16_t conn_handle, ble_lbs_t * p_lbs, uint8_t led_state)
{
    if (led_state)
    {
        nrf_gpio_pin_set(CONTROL_LED);
        NRF_LOG_INFO("Received LED ON from link 0x%x!", conn_handle);
    }
    else
    {
        nrf_gpio_pin_clear(CONTROL_LED);
...
int main(void)
{
    ...
    // Start execution.
    NRF_LOG_INFO("Primate Optogenetics started.");
